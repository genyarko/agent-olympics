...
const AGENTS = [
  { id: "orchestrator", name: "Orchestrator", icon: Settings, color: "text-purple-500", bg: "bg-purple-50" },
  { id: "researcher", name: "Researcher", icon: Search, color: "text-blue-500", bg: "bg-blue-50" },
  { id: "analyst", name: "Analyst", icon: BarChart3, color: "text-emerald-500", bg: "bg-emerald-50" },
  { id: "red_team", name: "Red Team", icon: ShieldAlert, color: "text-rose-500", bg: "bg-rose-50" },
  { id: "synthesizer", name: "Synthesizer", icon: FileText, color: "text-amber-500", bg: "bg-amber-50" },
];
  ...
  const startAnalysis = async () => {
    ...
    if (isDemoMode) {
      ...
      for (const event of MOCK_EVENTS) {
        ...
        if (event.agent === "orchestrator" && event.type === "status" && event.content === "done") {
    ...
    es.onmessage = (event) => {
      ...
      if (data.agent === "orchestrator" && data.type === "status" && data.content === "done") {
        closeStream();
        setIsAnalyzing(false);
      }
      if (data.type === "error") {
    ...
    await fetch(`/api/sessions/${currentSessionId}/analyze`, { method: "POST" });
  };
    ...
    <div className="flex flex-col h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest -mt-1">
              {uploadedFiles.length > 0 ? "Analysis in Progress: TargetCo" : "M&A Due Diligence Copilot"}
            </p>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
            ...
            <section>
              ...
              <div className="space-y-2">
                <form onSubmit={handleUrlAdd} className="relative">
                  ...
                  />
                  <button type="submit" disabled={!urlInput} className="absolute right-2 top-2.5">
                    <LinkIcon className={cn("w-3 h-3 transition-colors", urlInput ? "text-blue-500" : "text-slate-400")} />
                  </button>
                </form>
          ...
          <div className="p-4 border-t border-slate-100 bg-slate-50/50">
            <div className="flex items-center gap-2 mb-1">
              <div className={cn(
                ...
                backendStatus === "online" ? "bg-emerald-500" : (backendStatus === "connecting" ? "bg-amber-400" : "bg-rose-500")
              )} />
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">System Status</span>
            </div>
            <p className="text-xs text-slate-500 font-medium">
              {backendStatus === "online" 
                ? (sessionId ? `Session Active: ${sessionId.slice(0,8)}` : "Ready for new session")
                : (backendStatus === "connecting" ? "Checking connection..." : "Backend Offline - Try Demo Mode")}
            </p>
          </div>
        </aside>
        ...
        <main className="flex-1 flex flex-col overflow-hidden relative">
            ...
            <div className="grid grid-cols-6 h-full gap-6">
              ...
              <div className="col-span-2 flex flex-col h-full">
                <AgentPanel 
                  ...
                  allConflicts={Object.values(agentEvents).flat().filter(e => e.content.includes("⚡ Conflict"))}
                  isAnalyzing={isAnalyzing && (agentEvents[AGENTS[0].id]?.at(-1)?.type !== "status" || agentEvents[AGENTS[0].id]?.at(-1)?.content !== "done")}
              ...
              <div className="col-span-4 grid grid-cols-2 grid-rows-2 gap-6">
                {AGENTS.slice(1).map((agent) => (
                  <AgentPanel 
                    ...
                    allConflicts={Object.values(agentEvents).flat().filter(e => e.content.includes("⚡ Conflict"))}
                    isAnalyzing={isAnalyzing && (agentEvents[agent.id]?.at(-1)?.type !== "status" || agentEvents[agent.id]?.at(-1)?.content !== "done")}
          ...
          {brief && (
            <div className="absolute inset-x-6 bottom-6 top-16 bg-white shadow-2xl rounded-2xl border border-slate-200 z-30 flex flex-col animate-in slide-in-from-bottom-10 fade-in duration-700 print:relative print:inset-0 print:shadow-none print:border-none print:z-0">
              <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 rounded-t-2xl print:hidden">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium tracking-wide uppercase">Final Synthesized Recommendation</p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-12 print:p-0 print:overflow-visible">
                <div className="max-w-3xl mx-auto space-y-12">
                  ...
                  <div className="hidden print:block border-b-2 border-slate-900 pb-4 mb-8">
                    <h1 className="text-4xl font-black uppercase tracking-tighter text-slate-900">Boardroom Briefing</h1>
                    <p className="text-sm font-bold text-slate-500 mt-2">CONFIDENTIAL EXECUTIVE REPORT • {new Date().toLocaleDateString()}</p>
                  </div>
                  ...
                  <div className="flex items-start justify-between gap-8 p-8 bg-blue-50 rounded-2xl border border-blue-100 print:bg-white print:border-slate-200">
                    <div className="flex-1">
                      <span className="text-[10px] font-bold text-blue-500 uppercase tracking-widest block mb-2 print:text-slate-500">Verdict</span>
                      <h3 className="text-3xl font-black text-blue-900 mb-4 print:text-slate-900">{brief.recommendation}</h3>
                      <p className="text-blue-800/80 leading-relaxed font-medium print:text-slate-700">{brief.confidence_explanation}</p>
                    </div>
                    <div className="flex flex-col items-center gap-2 shrink-0">
                      <div className="w-24 h-24 rounded-full border-8 border-blue-200 flex items-center justify-center relative print:border-slate-200">
                         <div 
                          ...
                          style={{ clipPath: `inset(${100 - brief.confidence_score}% 0 0 0)` }}
                         />
                         <span className="text-2xl font-black text-blue-700 print:text-slate-900">{brief.confidence_score}%</span>
                      </div>
                      <span className="text-[10px] font-bold text-blue-400 uppercase print:text-slate-400">Confidence</span>
                    </div>
                  </div>

                  {/* Summary */}
                  <section>
                    <h4 className="text-sm font-bold text-slate-400 uppercase tracking-widest mb-4">Strategic Summary</h4>
                      ...
                      <ul className="space-y-4">
                        {brief.key_strengths.map((s, i) => (
                          <li key={i} className="flex gap-3">
                            ...
                            <div>
                              <p className="text-sm text-slate-800 font-medium leading-snug">{s.point}</p>
                              <p className="text-[10px] text-slate-400 font-mono mt-1">Ref: {s.source_citation}</p>
                            </div>
                          </li>
                        ))}
                      </ul>
                    ...
                    <section>
                      ...
                      <ul className="space-y-4">
                        {brief.key_risks.map((r, i) => (
                          <li key={i} className="flex gap-3">
                            <div className={cn("w-1.5 h-1.5 rounded-full mt-2 shrink-0", r.severity === "high" ? "bg-rose-600" : "bg-amber-500")} />
                            <div>
                              <p className="text-sm text-slate-800 font-medium leading-snug">{r.point}</p>
                              <div className="flex items-center gap-2 mt-1">
                                <span className={cn("text-[9px] px-1.5 py-0.5 rounded font-bold uppercase", r.severity === "high" ? "bg-rose-100 text-rose-600" : "bg-amber-100 text-amber-600")}>
                                  {r.severity}
                                </span>
                                <p className="text-[10px] text-slate-400 font-mono">Ref: {r.source_citation}</p>
                              </div>
                            </div>
                          </li>
                        ))}
                  ...
                  <section className="p-8 bg-slate-50 rounded-2xl border border-slate-200 print:bg-white">
                    <h4 className="text-sm font-bold text-slate-500 uppercase tracking-widest mb-4">Adversarial Perspective (Red Team)</h4>
                    <div className="space-y-4">
                      {brief.dissenting_views.map((d, i) => (
                        <p key={i} className="text-sm text-slate-600 font-medium leading-relaxed italic border-l-2 border-slate-300 pl-4">
                          {d}
                        </p>
                      ))}
                    </div>
                  ...
                  <section className="print:hidden">
                    <h4 className="text-sm font-bold text-blue-600 uppercase tracking-widest mb-6">Strategic Next Steps</h4>
                    <div className="grid grid-cols-3 gap-4">
                      {brief.follow_up_questions.map((q, i) => (
                        <div key={i} 
                          ...
                          <p className="text-xs text-slate-700 font-semibold group-hover:text-blue-600 transition-colors">{q}</p>
                          <div className="mt-4 flex items-center gap-1 text-[9px] font-bold text-blue-500 uppercase tracking-tighter">
                            Add to analysis <Play className="w-2 h-2 fill-current" />
                          </div>
                        </div>
                      ))}
                    </div>
...
function AgentPanel({ agent, events, allConflicts, isAnalyzing, scrollRef }: { 
  ...
  const status = events.length === 0 ? "idle" : (isAnalyzing ? "working" : "done");
  
  // Group events
  const toolCalls = events.filter(e => e.content.startsWith("Searching for:") || e.content.startsWith("Parsed:"));
  const conflicts = events.filter(e => e.content.includes("⚡ Conflict detected"));
  const relevantConflicts = allConflicts.filter(c => c.content.includes(agent.name) || c.agent === agent.id);
    ...
    <div className={cn(
      ...
      <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        <div className="flex items-center gap-3">
          ...
          <span className="font-bold text-slate-700 text-sm tracking-tight">{agent.name}</span>
        </div>
        <div className="flex items-center gap-2">
          {status === "working" && (
      ...
      >
        {events.length === 0 && !isAnalyzing && (
          <div className="flex flex-col items-center justify-center h-full opacity-20 grayscale">
            ...
            <p className="font-sans font-semibold italic">Awaiting instructions...</p>
          </div>
        )}
        ...
        {relevantConflicts.length > 0 && (
          <div className="space-y-2">
            {relevantConflicts.map((c, i) => (
              <div key={i} className="p-3 bg-rose-50 border border-rose-100 rounded-lg text-rose-700 animate-in zoom-in-95 duration-300 ring-2 ring-rose-200 ring-offset-2">
                <div className="flex items-center gap-2 mb-1">
                  <ShieldAlert className="w-3 h-3" />
                  <span className="font-bold uppercase tracking-tighter text-[9px]">Conflict Detected</span>
                </div>
                <p className="font-sans font-medium text-xs">{c.content.replace("⚡ Conflict detected: ", "")}</p>
              </div>
            ))}
          </div>
        )}
        ...
        {toolCalls.length > 0 && (
          <div className="space-y-1 opacity-80">
            ...
            {toolCalls.map((tc, i) => (
              <div key={i} className="bg-slate-50 border border-slate-100 p-2 rounded flex items-center gap-2 overflow-hidden">
        ...
        <div className="space-y-3">
          {regularEvents.map((e, i) => (
            <div key={i} className={cn(
              ...
              e.type === "error" && "text-rose-500 bg-rose-50 p-2 rounded border border-rose-100"
            )}>
        ...
        {status === "done" && findings.length > 0 && (
          <div className="mt-4 pt-4 border-t border-slate-100 animate-in slide-in-from-bottom-2 duration-500">
            ...
            <ul className="space-y-2">
              {findings.map((f, i) => (
                <li key={i} className="font-sans text-[10px] font-medium text-slate-700 flex gap-2">
